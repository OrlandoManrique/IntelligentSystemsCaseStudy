...
def calculate_warehouse_stats(df):
    ...
    if 'DEMAND' in df.columns:
        ...
        stats["Total Travel Cost"] = (df['dist'] * df['DEMAND']).sum()
        
    return stats

def _print_stats(stats, title):
    """
    Custom formatted print to match user requirement.
    """
    print(f"\n{'-'*10} {title} {'-'*10}")
    ...
    # 3. Cost (Optional)
    if "Total Travel Cost" in stats:
        print(f"\n{'Total Travel Cost':<25}: {stats['Total Travel Cost']:,.2f}")
